...
namespace AccessReelApp.Prototypes
{
    public class TmdbApiClient
    {
        ...
        public async Task GetReviewsForMoviesByName(string[] movieNames, int maxReviewsPerMovie)
        {
            foreach (var movieName in movieNames)
            {
                ...
                var searchRequest = new RestRequest("");
                searchRequest.AddHeader("accept", "application/json");
                searchRequest.AddHeader("Authorization", "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJhZWEzNjQwN2E5YzcyNWM4ZjgyMzkwZjdmMzAwNjRhMSIsInN1YiI6IjY1MDFlMGMwNmEyMjI3MDEzNzJkZTI2MCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.BWcS2MB1VOSfsN5-cQgmAV288mJSzZpxxj20jlfc5SE"); 
                searchRequest.AddParameter("query", movieName);
                searchRequest.AddParameter("language", "en-US");
                searchRequest.AddParameter("page", 1); // Start with page 1
                    ...
                    if (searchJson.ContainsKey("results") && searchJson["results"].HasValues)
                    {
                        ...
                        foreach (var result in results)
                        {
                            ...
                            while (reviewsFetched < maxReviewsPerMovie)
                            {
                                var reviewsRequest = new RestRequest("");
                                reviewsRequest.AddHeader("accept", "application/json");
                                reviewsRequest.AddHeader("Authorization", "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiJhZWEzNjQwN2E5YzcyNWM4ZjgyMzkwZjdmMzAwNjRhMSIsInN1YiI6IjY1MDFlMGMwNmEyMjI3MDEzNzJkZTI2MCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.BWcS2MB1VOSfsN5-cQgmAV288mJSzZpxxj20jlfc5SE"); 
                                reviewsRequest.AddParameter("language", "en-US");
                                reviewsRequest.AddParameter("page", page);
